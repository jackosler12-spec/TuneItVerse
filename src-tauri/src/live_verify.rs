//! Windowed live readback. Does not import flash.rs (avoids a module cycle).
//! UDS uses the ISO-TP helper — never raw ALFI payload on the VPW writer.
...
fn offset_for(addr: u32, written_len: usize) -> usize {
...
pub fn probe_live_windows(
    ...
    let mut out = Vec::new();
    let cal_addr: u32 = if fam.contains("EDC") || fam.contains("MED") || fam.contains("ME7") || fam.contains("DELPHI") {
        0x0008_0000
    } else {
        0x0002_0000
    };
    ...
    let want_vpw = fam.contains("P01") || fam.contains("P59") || fam.contains("GM") || fam == "AUTO" || fam.is_empty();
    if want_vpw {
                ...
                Ok(resp) => match parse_mode3c_response(&resp) {
                    Ok(data) if !data.is_empty() => {
                        ...
                        out.push(LiveWindow {
                            ...
                            offset: offset_for(*addr, written_len),
                            data,
                            method: "VPW Mode 3C".into(),
                        });
                    }
    ...
    let want_uds_kwp = fam.contains("EDC")
        || fam.contains("MED")
        || fam.contains("BOSCH")
        || fam.contains("ME7")
        || fam.contains("DELPHI")
    ...
    if want_uds_kwp {
        for (addr, label) in &starts {
            match crate::uds::read_memory_by_address(port, crate::uds::Alfi::ADDR4_SIZE2, *addr, 64, true) {
                Ok(data) if !data.is_empty() => {
        ...
        for (addr, label) in &starts {
            match crate::kwp::kwp_read_memory(port, *addr, 64) {
                Ok(data) if data.len() > 4 => {
                    ...
                    out.push(LiveWindow {
                        label: format!("{}_kwp", label),
                        offset: offset_for(*addr, written_len),
                        data,
                        method: "KWP 0x23".into(),
                    });
                }
...
pub fn compare_windows(written: &[u8], windows: &[LiveWindow], logs: &mut Vec<String>) -> Result<(u32, bool), String> {
    ...
    if windows.is_empty() {
        return Err("No Mode 23 / Mode 3C / KWP 0x23 windows. Kernel or UDS session required.".into());
    }
...
pub fn attempt_live_backup(
    ...
    if windows.is_empty() {
        return WindowBackup {
            ...
            bytes: 0,
            crc32: None,
            notes: "No Mode 23 / 3C windows. Not a full-image backup.".into(),
        };
    }
...
mod tests {
    ...
    #[test]
    fn compare_empty_windows_errors() {
        let err = compare_windows(&[1, 2, 3], &[], &mut vec![]).unwrap_err();
        assert!(err.contains("No Mode"));
    }

    #[test]
    fn compare_matching_window() {
        let written = vec![0u8; 128];
        let w = LiveWindow { label: "t".into(), offset: 0, data: vec![0u8; 32], method: "test".into() };
        let (crc, ok) = compare_windows(&written, &[w], &mut vec![]).unwrap();
        assert!(ok);
        assert_ne!(crc, 0);
    }
}
