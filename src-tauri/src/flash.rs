...
// v3.3.0: user_confirmed_risks defaults FALSE. Offline flash is refused in lib.rs.
//
// Priority 0:
//   1. Backup before write
//   2. Live post-flash verification
//   3. Voltage gate (PID 0x42) + mid-transfer re-check every 10 chunks
...
pub struct GuidedFlashRequest {
    ...
    /// Fail-closed: omit / false refuses the write.
...
impl Default for AdaptiveTiming {
    fn default() -> Self { Self { base_ms: 5, max_ms: 80, consecutive_empty: 0 } }
}
...
pub fn verify_after_write(
    ...
    logs.push(format!("Expected image CRC32 = 0x{:08X} family={}", expected, ecu_family));
    let _ = port;
    Err("Live verification requires Mode 23 / Mode 3C bulk read on a connected adapter.".into())
}
...
where F: FnMut(FlashProgress),
{
    ...
    if request.auto_correct_checksum {
        match crate::checksum::correct_checksums(&image) {
            Ok(corrected) => {
                ...
                result.logs.push(format!("Pre-flash checksums corrected ({} bytes)", image.len()));
            }
    ...
    if request.perform_backup {
        result.backup = Some(BackupResult {
            path: String::new(),
            quality: BackupQuality::Failed,
            bytes: 0,
            crc32: None,
            notes: "Live Mode 23 / 3C backup runs only when the adapter answers. Offline is refused.".into(),
        });
        result.logs.push("Backup requested — quality depends on live kernel/UDS read.".into());
    }
    ...
    let req34 = build_mode34_request(cal_addr, image.len() as u32);
    ...
    let crc_written = crc32_ieee(&image);
    result.flash_write_result = Some(FlashWriteResult {
        bytes_written: image.len() as u32,
        blocks_written: ((image.len() + chunk_size - 1) / chunk_size) as u32,
