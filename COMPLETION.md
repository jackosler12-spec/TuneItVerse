# TuneItVerse v3.21.0 — wire what v3.20 documented

v3.20 docs claimed the script runtime and live `flash-progress` events were on main. They were not.
...
4. `uds::download_image` took `FnMut(u32, u32)` while `flash.rs` passed a `Result`-returning closure. That is a type error. Mid-write J2534 Vbatt abort could not compile.
...
6. `with_port` is `pub(crate)` so `v312` voltage helpers compile by design, not by accident.
7. `analyze_log` is `pub(crate)` so the bench runtime can call it.
