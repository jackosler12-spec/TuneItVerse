...
TuneItVerse (JRTuners) — Industry-standard open ECU tuning platform.
...
- Notable libraries: serialport (Rust), tauri-plugin-fs, tauri-plugin-dialog, serde + serde_json, quick-xml
...
src-tauri/               Tauri (Rust) backend: ECU protocols, device drivers, tauri config
  Cargo.toml
  src/                   can.rs, dtc.rs, flash.rs, security.rs, xdf.rs, lib.rs, main.rs, etc.
reference/               Large collection of legacy C# source, DLLs, XML/XDF/bin databases and example ECU files
.github/                 GitHub workflow/metadata
```
How it fits together: The UI in `src/` provides the desktop interface. The Tauri Rust code in `src-tauri/` exposes native functionality (serial/CAN communication, file I/O, checksum/security algorithms, XDF parsing, flashing helpers) to the frontend via Tauri commands. The `reference/` folder stores legacy C# tools, device drivers, and many ECU mapping and binary example files used by the tools or for development/testing.
...
- The `reference/` directory contains many sample XDF, XML, BIN and DLL files used by the platform for decoding and verification.
