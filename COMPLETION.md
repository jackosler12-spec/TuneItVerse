...
| **v1.3.0 (this)** | **Complete family-aware auto_load coverage of ALL refined_map_addrs keys (torque_limiter + start_of_injection fully wired + all prior). Fallback community maps expanded to match. Tuning advisor extended for diesel limiters/SOI. Version sync 1.3.0. Index notes + docs. Full operational industry-leading free platform with zero missing map entries from the DB.** |
...
- Auto-load tables: P01 from real 16263425.xml addresses (spark/fuel/idle prioritized); EDC16/EDC17/MED17 community refined maps from DB refined_map_addrs (driver wish, IQ, boost, rail, VGT/smoke, ignition, VE, lambda, EGR, VVT, knock, torque limiter, SOI) — fully family aware via get_ecu_by_family + optional family_hint. **All keys now load.**
...
- Logging templates, tuning advisor (now includes diesel limiter/SOI guidance), audit log, protocol auto-detect
...
**v1.3.0 delivers a complete, fully operational, industry-leading free ECU tuning application for the supported platforms and protocols with zero gaps in the refined map auto-load path. Continue expanding the DB and maps as you dump more of your own vehicles. No more bullshit prices.**
