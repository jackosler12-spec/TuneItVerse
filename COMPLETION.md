...
2. `lib.rs` still registered a stub `list_script_helpers` that only printed Python CLI strings.
3. `guided_flash_pipeline` called `orchestrate_guided_flash(..., |_| {})`. The Flash bar only jumped when invoke returned.
4. Scripts UI said it was not an interpreter. HTML / status bar still said 3.17.0.
...
3. `guided_flash_pipeline` takes `AppHandle` and emits `flash-progress` `{percent, bytes_done, bytes_total, voltage_v, voltage_warn}` on every VPW chunk and UDS download tick. UI listens. J2534 Vbatt sag mid-write aborts the UDS path when the gate is on. VPW already abort-gates every 10 chunks.
4. `map_from_log_cmd` accepts optional CSV and imports it into the log buffer before the occupancy heatmap.
5. Compare-another-BIN keeps the second image so Scripts `compare` has a target.
