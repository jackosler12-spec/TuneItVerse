...
fn synthesize_uds_positive(request: &[u8]) -> Vec<u8> {
    ...
    match sid {
        ...
        0x28 => vec![0x68, request.get(1).copied().unwrap_or(0)],
        0x31 => {
        ...
        0x36 => vec![0x76, request.get(1).copied().unwrap_or(1)],
        0x37 => vec![0x77],
        0x11 => vec![0x51, request.get(1).copied().unwrap_or(0x01)],
        0x14 => vec![0x54],
        0x19 => vec![0x59, request.get(1).copied().unwrap_or(0x02), 0xFF],
        0x85 => vec![0xC5, request.get(1).copied().unwrap_or(0x01)],
        _ => vec![0x7F, sid, 0x11],
    }
}
...
impl Read for MockSerialPort {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        if self.read_queue.is_empty() {
            return Err(io::Error::new(io::ErrorKind::TimedOut, "mock serial: no data"));
        }
...
impl SerialPort for MockSerialPort {
    ...
    fn baud_rate(&self) -> SpResult<u32> { Ok(self.baud_rate) }
    ...
    fn set_baud_rate(&mut self, baud_rate: u32) -> SpResult<()> { self.baud_rate = baud_rate; Ok(()) }
    ...
    fn set_timeout(&mut self, timeout: Duration) -> SpResult<()> { self.timeout = timeout; Ok(()) }
    ...
    fn bytes_to_read(&self) -> SpResult<u32> { Ok(self.read_queue.len() as u32) }
    ...
    fn try_clone(&self) -> SpResult<Box<dyn SerialPort>> {
        Err(Error::new(ErrorKind::Unknown, "MockSerialPort cannot be cloned"))
    }
...
mod tests {
    ...
    fn scripted_response_overrides_auto() {
        ...
        let mut buf = [0u8; 32];
        let n = p.read(&mut buf).unwrap();
        assert!(String::from_utf8_lossy(&buf[..n]).contains("7F1022"));
    }
