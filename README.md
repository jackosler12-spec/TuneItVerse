...
**v1.3.0 Fully operational for core DIY + pro workflows:** serial connect, DTC diagnostics, live Mode-01 PIDs, P01 table auto-load from real XML addresses, EDC16/EDC17/MED17 community maps (fully family-aware from DB refined_map_addrs with complete coverage including torque_limiter + SOI), checksum correct (P01 + multipoint), guided flash Mode 34/36/37 with backup/kernel/verify + UDS ready, ECU DB (P01, EDC16C41, P59, MED17, EDC17 fully refined) + get_ecu_info, **J2534 production path (list/connect/write/read fully registered)**, native table editor (grid/3D/hex).
