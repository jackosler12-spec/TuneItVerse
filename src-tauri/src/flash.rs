...
// v2.1: GuidedFlashRequest now accepts frontend aliases (bin_bytes, do_backup) + defaults for full UI compatibility.
...
impl AdaptiveTiming {
    ...
    pub fn for_vpw_hs() -> Self { Self { base_ms: 3, max_ms: 40, consecutive_empty: 0 } }
    ...
    match read_battery_voltage(port) {
        ...
        None => {
            if min_v <= 0.0 {
                ...
                Err("Voltage gate FAILED: could not read PID 0x42. Set min_voltage_v = 0 to bypass on controlled bench setups only.".into())
            }
        }
    }
}
...
pub fn kernel_is_alive(port: &mut Box<dyn SerialPort + Send>) -> bool {
    ...
    match request_response(port, &req) {
        Ok(resp) => !resp.is_empty() && (resp.iter().any(|&b| b == 0x7F || b == 0x3F) || resp.len() > 3),
        Err(_) => false,
    }
}

pub fn kernel_read_block(port: &mut Box<dyn SerialPort + Send>, address: u32, size: u16) -> Result<Vec<u8>, String> {
    let req = build_mode3c_read_block(address, size);
    let resp = request_response(port, &req).map_err(|e| format!("Mode 3C @ 0x{:06X}: {}", address, e))?;
...
fn probe_mode3c(port: &mut Box<dyn SerialPort + Send>, logs: &mut Vec<String>) -> bool {
    match kernel_read_block(port, 0x0000_0000, 0x10) {
        Ok(data) if !data.is_empty() => { logs.push(format!("Mode 3C probe OK ({} bytes)", data.len())); true }
        Ok(_) => { logs.push("Mode 3C probe returned empty — kernel may be limited".into()); false }
        Err(e) => { logs.push(format!("Mode 3C probe failed: {}", e)); false }
    }
}
...
pub fn try_enter_high_speed(port: &mut Box<dyn SerialPort + Send>, logs: &mut Vec<String>) -> bool {
    ...
    match request_response(port, &prep) {
        Ok(resp) => match parse_hs_response(&resp) {
            HsResponse::PrepareOk => { logs.push("Mode 0xA0 HighSpeedPrepare accepted (E0)".into()); }
            HsResponse::Negative => { logs.push("Mode 0xA0 rejected (NRC) — staying normal speed".into()); return false; }
            other => {
                if resp.is_empty() {
                    logs.push(format!("Mode 0xA0 unclear ({:?}) — aborting HS", other));
                    return false;
                }
                logs.push(format!("Mode 0xA0 response ambiguous ({:?}) — continuing to 0xA1", other));
            }
        },
        Err(e) => { logs.push(format!("Mode 0xA0 no response: {} — normal speed", e)); return false; }
    }
    ...
    match request_response(port, &enter) {
        Ok(resp) => match parse_hs_response(&resp) {
            HsResponse::EnterOk => {
                ...
                if crate::j2534::try_set_vpw_high_speed() {
                    logs.push("J2534 DATA_RATE → 41600 (VPW 4× physical layer)".into());
                } else if crate::j2534::is_device_open() {
                    logs.push("J2534 open but DATA_RATE 41600 failed — software HS only".into());
                }
                true
            }
            HsResponse::Negative => { logs.push("Mode 0xA1 rejected — staying normal speed".into()); false }
            other => {
                if resp.is_empty() {
                    logs.push(format!("Mode 0xA1 unclear ({:?}) — normal speed", other));
                    false
                } else {
                    logs.push(format!("Mode 0xA1 soft-accept ({:?}) — trying HS blocks", other));
                    if crate::j2534::try_set_vpw_high_speed() {
                        logs.push("J2534 DATA_RATE → 41600 (soft-accept path)".into());
                    }
                    true
                }
            }
        },
        Err(e) => { logs.push(format!("Mode 0xA1 no response: {} — normal speed", e)); false }
    }
}
...
where F: FnMut(FlashProgress),
{
    logs.push(format!("P01 kernel bulk read: base=0x{:06X} size={} prefer_hs={}", base, total_size, prefer_hs));

    if !kernel_is_alive(port) {
        logs.push("Kernel not responding — uploading Kernel-P01.bin…".into());
        upload_kernel(port, KERNEL_P01).map_err(|e| format!("Kernel upload failed: {}", e))?;
        if !kernel_is_alive(port) {
            return Err("Kernel upload completed but Mode 3F probe failed. Check L2 unlock, power, and VPW link.".into());
        }
    ...
    let mut timing = if high_speed { AdaptiveTiming::for_vpw_hs() } else { AdaptiveTiming::for_vpw() };
    ...
    while offset < total_size {
        ...
        match kernel_read_block(port, base + offset, this_size) {
            Ok(chunk) => {
                ...
                if got == 0 { consecutive_fail += 1; }
                else { out.extend_from_slice(&chunk); offset += got; }
            ...
            Err(e) => {
                timing.on_empty();
                consecutive_fail += 1;
                logs.push(format!("Mode 3C @ 0x{:06X} failed: {} (#{})", base + offset, e, consecutive_fail));
                if high_speed && !hs_fallback_done && consecutive_fail >= 3 {
                    logs.push("HS Mode 3C unstable — falling back to normal-speed blocks".into());
                    high_speed = false;
                    hs_fallback_done = true;
                    block_size = MODE3C_BLOCK;
                    timing = AdaptiveTiming::for_vpw();
                    consecutive_fail = 0;
                    if crate::j2534::try_set_vpw_normal_speed() {
                        logs.push("J2534 DATA_RATE → 10400 (HS fallback)".into());
                    }
                    continue;
                }
    ...
    if crate::j2534::try_set_vpw_normal_speed() {
        logs.push("J2534 DATA_RATE → 10400 (restored after ExitKernel)".into());
    }

    if out.is_empty() {
        return Err("Kernel bulk read returned no data".into());
    }
    logs.push(format!("Kernel bulk read recovered {} / {} bytes (HS used: {})", out.len(), total_size, high_speed || hs_fallback_done));
    Ok(out)
}
    ...
    while offset < total_size {
        ...
        match uds_read_memory_by_address(port, base + offset, this_win) {
            ...
            Err(e) => {
                timing.on_empty();
                consecutive_fail += 1;
                logs.push(format!("Mode 23 @ 0x{:08X} failed: {} (#{})", base + offset, e, consecutive_fail));
...
pub fn perform_backup(port: &mut Box<dyn SerialPort + Send>, ecu_family: &str, logs: &mut Vec<String>) -> BackupResult {
    ...
    if is_bosch {
        ...
        match bulk_read_memory(port, BOSCH_FLASH_BASE, BOSCH_FLASH_SIZE, MODE23_WINDOW, |_| {}, logs) {
            Ok(data) => {
                ...
                let quality = if data.len() as u32 >= BOSCH_FLASH_SIZE { BackupQuality::FullImage }
                    else if data.len() >= 64 { BackupQuality::PartialDidOnly } else { BackupQuality::Failed };
    ...
    if is_p01 {
        ...
        match p01_kernel_bulk_read(port, 0x0000_0000, P01_FLASH_SIZE, |_| {}, logs) {
            Ok(data) => {
                let crc = crc32_ieee(&data);
                let quality = if data.len() as u32 >= P01_FLASH_SIZE { BackupQuality::FullImage }
                    else if data.len() as u32 >= P01_CAL_SIZE || data.len() >= 256 { BackupQuality::PartialDidOnly }
                    else { BackupQuality::Failed };
                let path = if quality == BackupQuality::FullImage { format!("p01_full_{}.bin", ts) } else { format!("p01_partial_{}.bin", ts) };
                let _ = std::fs::write(&path, &data);
                logs.push(format!("P01 backup {} — {} bytes CRC=0x{:08X} quality={:?}", path, data.len(), crc, quality));
                return BackupResult {
                    path, quality, bytes: data.len() as u32, crc32: Some(crc),
                    notes: match quality {
                        BackupQuality::FullImage => "Full 512 KB P01 image via kernel Mode 3C (±HS). Restore-grade.".into(),
                        BackupQuality::PartialDidOnly => format!("Partial kernel dump ({} of {} bytes). Not full restore-grade.", data.len(), P01_FLASH_SIZE),
                        BackupQuality::Failed => "Kernel dump too small.".into(),
                    },
                };
            }
            Err(e) => { logs.push(format!("Kernel bulk read failed ({}), falling back to Mode 22 partial…", e)); }
        }
    }
        ...
        match request_response(port, &req) {
            Ok(resp) => {
                ...
                for (j, b) in resp.iter().take(128).enumerate() {
                    let idx = i * 128 + j;
                    if idx < backup_data.len() { backup_data[idx] = *b; filled += 1; }
                }
            }
            Err(_) => { timing.on_empty(); if timing.consecutive_empty > 8 { break; } timing.sleep(); }
        }
    }
    ...
    BackupResult {
        path, quality: BackupQuality::PartialDidOnly, bytes: backup_data.len() as u32, crc32: Some(crc),
...
pub fn verify_after_write(
    ...
    if is_bosch {
        ...
        let base = if written.len() as u32 >= BOSCH_FLASH_SIZE { BOSCH_FLASH_BASE } else { 0x0008_0000 };
        let readback = bulk_read_memory(port, base, window_total, MODE23_WINDOW, |_| {}, logs)?;
        if readback.len() < 256 { return Err("Live verification failed: insufficient readback".into()); }
        ...
        logs.push(format!("Live CRC ({} bytes): ECU=0x{:08X} expected=0x{:08X} match={}", compare_len, live_crc, expected_window, matched));
        if !matched { return Err(format!("LIVE VERIFICATION MISMATCH: 0x{:08X} != 0x{:08X}", live_crc, expected_window)); }
    ...
    if is_p01 {
        ...
        if readback.len() < 256 { return Err("P01 live verify: insufficient kernel readback".into()); }
        let compare_len = readback.len().min(written.len());
        let live_crc = crc32_ieee(&readback[..compare_len]);
        let expected_window = crc32_ieee(&written[..compare_len]);
        let matched = live_crc == expected_window;
        logs.push(format!("P01 live CRC ({} bytes): ECU=0x{:08X} expected=0x{:08X} match={}", compare_len, live_crc, expected_window, matched));
        if !matched { return Err(format!("LIVE VERIFICATION MISMATCH: 0x{:08X} != 0x{:08X}", live_crc, expected_window)); }
        return Ok((live_crc, true));
    }
...
where F: FnMut(FlashProgress),
{
    ...
    if request.perform_backup {
        ...
        result.steps_completed.push(format!("Backup finished (quality={:?}, {} bytes)", backup.quality, backup.bytes));
    ...
    if request.auto_correct_checksum && request.tuned_bin.len() == CAL_IMAGE_SIZE as usize {
        match correct_and_validate_checksums(&request.tuned_bin) {
            Ok(corrected) => { result.checksum_report = Some(corrected.report.clone()); result.logs.push("Pre-flash checksums corrected".into()); }
            Err(e) => { result.error = Some(e); return Ok(result); }
        }
    }
