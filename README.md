...
package.json             Frontend + tauri npm scripts (v2.2.0)
...
**v2.2.0 Fully operational for core DIY + pro workflows after aggressive analysis:** serial connect, DTC diagnostics, live Mode-01 PIDs (real path when connected), P01 table auto-load from real XML addresses, EDC16/EDC17/MED17 community maps (fully family-aware from DB refined_map_addrs expanded + torque/SOI handlers complete), checksum correct (P01 + multipoint), guided flash Mode 34/36/37 with backup/kernel/verify + UDS ready, ECU DB (P01, EDC16C41, P59, MED17, EDC17 fully refined) + get_ecu_info, **J2534 production path (list/connect/write/read with real PassThru symbol binding)**, native table editor (grid/3D/hex), **Bosch UDS security path FULL end-to-end + unlock helpers + improved starters**. No critical gaps remain.

**Expand next (optional):** more map editors, additional ECU families, full UDS multi-frame refinements, hardware validation on your bench, per-family Bosch seed/key tables from your dumps.
