...
pub fn correct_checksums(data: &[u8]) -> Result<CorrectedCal, String> {
    ...
    let all_valid = failed_count == 0;
    let report = ChecksumReport { regions: results, valid_count, fixed_count, failed_count, all_valid };
...
mod tests {
    ...
    fn valid_block() -> Vec<u8> {
        ...
        for region in &REGIONS { let _ = correct_region(&mut block, region); }
        block
    }
